...
void sendSettingsData() {
	events.send(Stgs.getValuesJSON_String().c_str(), "new_settings", millis());
}
...
void initWebServer() {
	...
	server.on("/readings", HTTP_GET, [](AsyncWebServerRequest *request) {
		String json = ActlVal.getValuesJSON_String();
...
//		Serial.println(String(json));
	...
	server.on("/settings", HTTP_GET, [](AsyncWebServerRequest *request) {
		String json = Stgs.getValuesJSON_String();
		request->send(200, "application/json", json);
#ifdef DEBUG
...
//		Serial.println(message);
	...
	// Сброс всех настроек.
	server.on("/settings_reset_req", HTTP_GET, [](AsyncWebServerRequest *request) {
		Stgs.ResetSettings();
		Stgs.reboot();
...
void WebEvents() {
	if (timerWebSrvEvents_readings.ready()) {
		...
		events.send(ActlVal.getValuesJSON_String().c_str(), "new_readings", millis());
	}
}
